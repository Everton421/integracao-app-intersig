...
export async function serviceSendClient(event: event) {
        ...
        let sql = ` select *,
                                                ...
                                                c.ativo = 'S' 
                                                `
        const [resultVerifyClient] = await dbConn.query(`SELECT  id_mobile, codigo_sistema FROM ${MOBILE}.clientes_enviados where codigo_sistema = ${event.id_registro};`);
        ...
        if (arrVerifyClient.length > 0) {
                ...
                const data = {
                        ...
                        id: String(clientVerify.codigo_sistema),
                ...
                const resultPut = await api.put("/clientes", data,
                        {
                                headers: {
                                        source: origin
                                }
                ...
                const resultPost = await api.post("/clientes", data,
                        {
                                headers: {
                                        source: origin
                                }
