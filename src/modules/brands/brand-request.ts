...
export class BrandRequest {
    ...
    static async post(brand: inputPostbrand) {
        ...
        try {
            ...
            const resultPost = await api.post("/marcas", payload,
                {
                    headers: {
                        source: new BrandRequest().origin
                    }
                }
            )
            ...
            if (isAxiosError(e)) {
                ...
                resultfunction.message = `[x] Erro inesperado ao tentar registrar marca na api, payload ${brand}. ${e} `;
            }
    ...
    static async put(brand: Omit<inputPostbrand, 'codigo'>, id_mobile: number) {
        ...
        try {
            ...
            const resultPut = await api.put("/marcas", payload,
                {
                    headers: {
                        source: new BrandRequest().origin
                    }
            ...
            if (isAxiosError(e)) {
                ...
                resultfunction.message = `[x] Erro inesperado ao tentar atualizar marca na api, payload ${brand}. ${e} `;
            }
    ...
    static async delete(codigo:number){
        ...
        try {
        const resultDelete = await api.delete(`/marcas/${codigo}`, 
                {
                    headers: {
                        source: new BrandRequest().origin
                    }
           ...
           if(resultDelete.status === 200 ){
            ...
            if (isAxiosError(e)) {
                ...
                resultfunction.message = `[x] Erro inesperado ao tentar atualizar marca na api, codigo: ${codigo}. ${e} `;
            }
