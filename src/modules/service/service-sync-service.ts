...
export class ServiceSyncService {
    static async syncData(event: event) {
        ...
        try {
            ...
            const [resultVerifySended] = await dbConn.query(`SELECT * FROM ${MOBILE}.servicos_enviados WHERE codigo_sistema = '${event.id_registro}';`)
            ...
            if (event.tipo_evento === 'UPDATE') {
                if (arrVerifySended.length > 0) {
                    ...
                    const resultUpdate = await ServiceUpdateService.update(event.id_registro, id_mobile)
            ...
            if (event.tipo_evento === 'INSERT') {
                if (arrVerifySended.length > 0) {
                    ...
                    resultFunction.message = `[X] servico[ERP] ${event.id_registro} já foi enviado(a).`
                } else {
                    const resultServiceSend = await ServiceSendService.send(event.id_registro);
            ...
            if (event.tipo_evento === 'DELETE') {
                ...
                resultFunction.message = `[X] DELETE ignorado para servico[ERP] ${event.id_registro}.`
            }
