...
test("UpdateSalesOrderSeparation" , async ( t )=>{
    ...
    await t.test("updateErpOrder ", async ()=>{
        const resultUpdateSalesOrder = await UpdateSalesOrderSeparation.updateErpOrder( { pedido :9, tipo :1, situacao_separacao: 'I', itens_processados :1, series_registradas :1})
